...
拓展可以在自己目录旁挂 `module/panel.json`，网页端会据此在该拓展详情区生成一个「用户配置」页签，用来展示状态、修改配置和触发少量动作。
...
└── module/                  # 可选，用户配置面板
    ├── panel.json           # 面板声明
    ├── panel.values.json    # 配置值，由框架读写
    ├── status.json          # 状态数据，由模块写入
    └── README.md
```

面板与 `input_data.md` 是**平行链路**：面板里的配置值和状态**不会自动进入提示词**、记忆、历史或知识库。只有模块自己更新原有数据文件，才可能按原规则影响提示词。

### 三种容器

| 容器 | 用途 |
|---|---|
| `status` | 只读展示，从 `status.json` 取值 |
| `config` | 读写配置，保存写入 `panel.values.json`，支持预设一键套用 |
| `action` | 触发动作；拓展走既有 `expand_call` 通道 |

输入字段支持 `string`、`number`、`boolean`、`enum`、`text`；状态展示支持 `text`、`badge`、`keyvalue`、`markdown`。未声明的 key、类型错误、超范围数字、非法枚举和必填缺失都会被拒绝；配置 key 与动作命令在面板内必须唯一。

### 密钥字段

标记为 masked 的字段可以明文保存在模块自己的值文件里，但接口与悬浮预览只返回「已设置 / 未设置」，明文不下发浏览器。保存时省略该字段会保留旧值，只有显式清空才会删除。密钥不得写入 `input_data.md`。

### 布局与边界

面板使用 12 列网格，卡片宽度四档（1/4、1/3、1/2、全宽），高度两档；内容溢出时在卡片内部滚动，悬停或键盘聚焦可查看完整预览。

面板完全由框架渲染，**不接受模块自带的 HTML、CSS、JavaScript、iframe 或事件处理器**。面板只声明允许操作的命令，模块入口必须再次做权限与失败语义校验。

新增或修改 `panel.json` 无需重启后端，下一次打开面板或刷新页面即可生效。
