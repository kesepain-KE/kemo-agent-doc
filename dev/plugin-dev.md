...
必填字段是 `name`、`description`、`input_schema`、`version`、`enabled`、`entrypoint`。`input_schema.type` 必须为 `object`；入口必须使用同目录 `file.py:function`，不能路径逃逸。
...
参数会先按 JSON Schema 校验。执行受 `tools.timeout`、最大迭代数、连续相同调用限制和取消信号控制。
