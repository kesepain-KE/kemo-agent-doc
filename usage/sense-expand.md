...
拓展和感知都可以在模块目录旁挂一个配置面板声明，网页端会据此在模块详情里生成「用户配置」区域：状态只读展示、配置可读写、动作按钮触发有限操作。

面板与数据注入是**平行链路**——面板里的值不会自动进入提示词，只有模块自己更新原有数据文件才可能生效。声明方式见[拓展开发](/dev/expand-dev)与[感知开发](/dev/sense-dev)。
