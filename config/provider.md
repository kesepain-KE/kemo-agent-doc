...
同样的能力边界适用于运行中引导。Kemo 只会直传模型能力声明包含的模态；Chat 只可能直传图片。未直传的音频、视频和普通文件仍会登记到当前 Run，由专用多模态工具或文件工具处理，不会被静默删除。
...
Kemo Gateway `0.7.0` 起已支持 Asset 上传与检索（`POST /v1/assets`、`GET /v1/assets/{id}`），
