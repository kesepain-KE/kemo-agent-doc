...
- 重启服务不再与终端绑定：Windows 上替换进程改用窗口版解释器 `pythonw.exe`。此前服务进程一旦因任何路径持有控制台，重启后会带出一个终端窗口，而那个窗口本身就是服务进程——关掉它等于停掉服务；改为不持有控制台的解释器后，关闭任何终端都不再影响服务，替换进程的输出仍完整写入 `update/runtime/restart.log`。
