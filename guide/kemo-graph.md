...
> 当前版本：v1.5.0 — 更新入口新增只读检查 `--dirty/--changes` 与强制更新 `--force`（先备份未提交的程序文件修改，失败时放回原处），更新被拒绝时附带可执行的下一步提示（v1.4.0 起文档项目化组织、独立构建状态页、真实检索阶段遥测、分类运行日志与有界读取缓存；v1.3.1 起导入快照哈希校验（`expected_origin_hash` 与 `IMPORT_SOURCE_CHANGED`）、多编码文本识别增强、网页端知识文档 Markdown/KaTeX/Mermaid 预览渲染；v1.3.0 起可调图谱抽取与稳健召回、KnowledgeBaseService 服务化、同版本强制更新统一、本地 markitdown 文档归一化；v1.2.1 起 Store multipart 文件上传导入与同版本强制更新；v1.2.0 起外部权威来源同步协议、Office/EPUB/RTF 与结构化数据转换、GPU 优先图谱渲染；v1.1.1 起语义叶子规范化与检索层级族折叠；v1.1.0 起查询规划、语义分层切分、可移植知识库与应用更新系统）。
...
v1.5.0 把「工作区有未提交修改」从死路变成可选路径：

- `python update.py --dirty`（别名 `--changes`，或 `python start.py update-changes`）只读列出未提交的更改，并把**会阻塞更新的程序文件**与**不影响更新的用户配置、运行数据**分开列出；该命令不联网、不写入更新状态。
- `python update.py --force` 在存在未提交的程序文件修改时，先把它们备份到 `update/runtime/dirty-backup/<时间戳>/` 并还原工作区，再继续安装；安装失败会把这些修改放回原位置。它同时保留原有的「同版本强制重装」语义。
- 更新被拒绝时，返回的错误对象附带 `hint` 字段，直接给出查看未提交更改与强制更新的命令。
- `config/config.json`、`.env`、知识库数据、外部文档、日志与输出目录始终属于受保护范围，既不阻塞更新，也不会被覆盖。
- 编辑器与手动备份文件（`*.bak`、`*.orig`）不再被计为未提交的程序文件修改。

升级到 1.5.0 后才能使用这两个选项；旧版更新入口在检测到未提交的程序文件修改时仍然直接拒绝。`--force` 会临时移除工作区中的未提交修改，需要对照原始内容时保留备份目录即可。

## 持续集成

仓库提供 GitHub Actions 工作流 `CI`，在推送到 `main` 与合并请求时执行发布前检查：

- 后端在 Python 3.10 与 3.12 两个版本上安装 `requirements-dev.txt`，运行语法检查与全部单元测试；
- 前端执行类型检查、单元测试与生产构建。

检查项与发布前的验证命令保持一致，因此新克隆的代码在合并前就能暴露环境差异类问题（例如被忽略规则误排除的源码文件、对本地构建产物的隐式依赖）。知识库的符号链接边界防护用例在无法创建链接的平台自动跳过，由 Linux 运行器覆盖。
