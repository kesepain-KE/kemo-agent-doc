...
## 工具参数执行边界与流式原子发布（0.7.6）

`0.7.6` 收紧 Provider 工具调用的执行边界，不新增公开 API：

- Provider 工具参数在执行前必须解析为完整 JSON 对象并通过请求工具 Schema 校验；非法、缺失、空字符串、非对象或深层超限参数不会执行，统一以有限诊断和 `response.incomplete` 表示。
- 流式 `tool_call.completed` 事件会等统一终态通过校验后再按 `call_id` 发布；并行调用按批次原子处理，同批任意调用非法时不提前发布或执行其他调用。
- Schema 校验增加递归深度、节点总数和数组项数限制，避免异常输入造成递归崩溃或无界扫描。
- 保留既有多密钥故障转移、热配置失败回滚、公网鉴权和诊断脱敏规则。

## Provider 生命周期与热配置快照（0.7.7）

`0.7.7` 补齐运行中的 Provider 包生命周期，不改变 kemo-agent 的调用路径：

- Provider 目录被删除时，已准入执行继续完成，新请求与管理台立即拒绝路由到已不存在的厂商。
- 热配置快照把 Provider 目录本身作为指纹标记：目录整体删除或新增即使没有 config.json/secrets.json 也会触发重载。
- Web 控制台对 Provider 诊断失败保持可用，不会因单个陈旧包返回 HTTP 500。

## 更新器计划预览与安全边界（0.7.8）

`0.7.8` 只增强部署端更新控制面，Kemo Protocol 仍为 `1.0`：

- 更新菜单可预览计划、查看源码状态；`--apply --dry-run` 不改工作树、配置或运行数据。
- `--repo-url` / `--branch` 不改写本地 origin；`--remote-version-url` 只用于 Git 不可用时的只读版本查看。
- 更新写入期间使用跨平台锁和 `.update.maintenance` 标记；Git 与命令错误自动脱敏。

## 智能体引导、统一测试与统计读缓存（0.8.0）

`0.8.0` 聚焦可维护性与管理体验，Kemo Protocol 仍为 `1.0`：

- **按任务引导智能体**：创建厂商、增加模型、修改能力、单模型多模态、推理档位和密钥轮换分别提供步骤、样例与完成判据，避免直接用模板覆盖已有实现；仓库同时提供智能体配方索引与步骤手册，便于按任务定位操作。
- **模板离线门禁**：检查模型集合、manifest、能力声明与推理映射的一致性，九种操作提供配对请求样例。模板通过只证明结构，不代表目标厂商真实可用。
- **统一测试入口**：`python -m tests` 默认运行源码与模板测试并按职责分组；本地厂商与真实重启测试需要显式选择，共享构造器不再依赖其他测试文件。
- **有界统计读缓存**：日/小时统计、趋势、排行、密钥累计用量与日志分页复用短时结果，上限 128 项、8 MiB 序列化载荷、30 秒 TTL。统计写入后立即失效，调用记录与 SSE 仍按原有事务落盘。
- **管理界面与安全预览**：优化卡片层次并增加只读网关配置页；认证与密钥、Provider 密钥池显示后端生成的前五后三掩码，短值完全隐藏，不回传完整凭据或私有 JSON。
- **共享线路契约门禁**：网关与 kemo-agent 镜像同一份脱敏线路样例，两边分别使用自己的生产协议模型、序列化器和 SSE 解析器执行，不跨仓库导入代码。基准固定 SHA-256，可用 `--peer-root` 与对端逐字节核对；仓库用 `.gitattributes` 固定基准文件以 LF 检出，避免 Windows 换行转换让固定摘要失配。
- **协议校验收敛**：`protocol_version` 只接受 `MAJOR.MINOR` 形式并拒绝其他主版本；资产内容的 `checksum_sha256` 要求 64 位十六进制并统一小写。

升级不需要迁移现有密钥文件；网关与前端管理包统一为 `0.8.0`，源码与前端变更需要重新构建并重启生效。`0.7.x` 的更新器计划预览、备份与安全快进，以及 Provider 热配置和在途请求保护继续保留。

## SSE 执行日志持久化（0.8.1）

`0.8.1` 优化流式执行的日志落盘路径，不新增公开 API，Kemo Protocol 仍为 `1.0`：

- **按响应有界缓冲**：非终态 SSE 事件先进入该响应独立的内存队列，默认在 50 ms、32 条事件或统一终态任一条件达到时，以一个 SQLite 事务批量提交；不同响应的队列完全隔离。
- **只改事务粒度**：不合并、不删除 Kemo 事件，每条事件仍保留独立 `event_id` 与连续 `sequence`。
- **先提交、后发布**：批次 `COMMIT` 成功后事件才进入可重放列表并通知 SSE 客户端，因此客户端已经收到的事件必然位于持久化边界内；提交前进程崩溃时，内存中的事件既不会恢复，也从未发布。
- **终态不等待窗口**：统一终态、取消与正常关停会立即 flush 待写事件，并在同一事务内提交终态、最终响应和执行状态。
- **连接复用与序号热路径**：网关生命周期内复用受锁保护的 SQLite WAL 连接，正常关闭时 checkpoint 并截断 WAL；`sequence` 由内存中的已持久化序号维护，数据库复合主键与 event ID 唯一约束继续兜底。
