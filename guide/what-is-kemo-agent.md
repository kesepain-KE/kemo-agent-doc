...
项目当前正式版本为 `1.3.1`。`1.0.0` 标志着主生态首次补齐并进入稳定主版本；`1.1.0` 完成主生态的移动端闭环，新增 `kemo_app` 全局拓展与 Android 客户端；`1.1.1` 将 Android App 会话独立为 `source=app` 分区，与 Web 历史完全隔离，并让更新器接管桥接模块的平滑升级；`1.1.2` 强化任务计划运行边界（创建后当前会话收束、对话空间隔离、执行器原子领取），新增前台条件等待工具，并修复 Web 暗色主题；`1.2.0` 引入会话级长任务模式——用户为单个对话空间显式开启后，达到单轮工具调用上限时自动跨 Run 继续执行，压缩过程也在输入框上方可见。

`1.2.1` 到 `1.2.9` 是一串以稳定性为主的版本：核心目录化拆分、工具参数与后台进程边界、外部 Agent 桥接与调度解耦、删除栅栏与作用域隔离、运行环境隔离与跨平台 Shell，以及 Chat 兼容链路的传输加固和子代理进度上报。其中 `1.2.9` 改造了记忆的提取与晋升——碎片按「拆开之后是否还能独立理解」分档，提取条数改按独立事实计算，超过档位上限时在同事务内先拆后存。

`1.3.0` 在长期记忆、会话生命周期、重试边界与模块扩展四个方面收口：加权证据绑定原轮次的上海自然日，可追溯且重复提交幂等；命令行、定时任务、网页与 App 四条链路的会话收尾对齐；重试收敛为运行级预算，一次中断最多重试五次；拓展与感知可挂载 `module/panel.json` 生成声明式用户配置面板，回复正文也可直接嵌入 `kemo-widget` 声明式组件。当前已确认与 `kemo-adapter-api 0.8.2` 完成 Kemo 1.0 线路协议匹配。`1.3.1` 新增四条一键部署入口（Windows、Linux、npm、Docker），并把这些入口背后的部署器从框架里独立出来——它不导入主框架、只依赖 Python 标准库，能按映射把发布包铺到目标目录，同时原样保留用户数据与凭据。后续版本将继续聚焦边缘生态、性能与长期可靠性。

接下来可以阅读 [v1.3.1 更新说明](/releases/release-1.3.1)，或直接进入[快速开始](/guide/getting-started)。
