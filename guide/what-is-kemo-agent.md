...
项目当前正式版本为 `0.3.0`。核心运行、Web 操作台、多模态路由、分层记忆、任务调度、外部消息和模块化更新已经形成完整闭环；本版进一步完善了 Web 认证、后台历史摘要、记忆批处理、工具与子代理超时边界，并完成运行模块解耦。项目仍处于早期阶段，兼容性与扩展体验会继续演进。

接下来可以直接阅读[快速开始](/guide/getting-started)。
