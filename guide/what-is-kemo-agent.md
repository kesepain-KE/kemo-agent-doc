...
项目当前正式版本为 `0.8.0`。核心运行、Web 操作台、多模态路由、分层记忆、任务调度、外部消息和模块化更新已经形成完整闭环；本版本让 Kemo 模型的思考档位跟随网关能力声明动态变化，并为智能体创建的子代理、拓展、消息路由、感知、技能和用户包补齐独立合同验收基准。项目仍处于早期阶段，兼容性与扩展体验会继续演进。

接下来可以阅读 [v0.8.0 更新说明](/releases/release-0.8.0)，或直接进入[快速开始](/guide/getting-started)。
