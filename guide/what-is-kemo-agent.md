...
项目当前正式版本为 `0.7.0`。核心运行、Web 操作台、多模态路由、分层记忆、任务调度、外部消息和模块化更新已经形成完整闭环；本版统一收录 2026 年 7 月 28 日的全部改动，新增媒体预览、用户技能 ZIP 安装、跨层记忆检索和 Kemo 网关状态能力，并完成 Web 后端领域分层与多处界面改进。项目仍处于早期阶段，兼容性与扩展体验会继续演进。

接下来可以阅读 [v0.7.0 更新说明](/guide/release-0.7.0)，或直接进入[快速开始](/guide/getting-started)。
