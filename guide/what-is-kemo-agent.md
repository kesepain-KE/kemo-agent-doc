...
项目当前正式版本为 `0.6.0`。核心运行、Web 操作台、多模态路由、分层记忆、任务调度、外部消息和模块化更新已经形成完整闭环；本版进一步完善了 Kemo 模型目录选择、网关只读状态拓展和协议续轮兼容。项目仍处于早期阶段，兼容性与扩展体验会继续演进。

接下来可以阅读 [v0.6.0 更新说明](/guide/release-0.6.0)，或直接进入[快速开始](/guide/getting-started)。
