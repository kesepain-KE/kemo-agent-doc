...
项目当前正式版本为 `0.10.0`。核心运行、Web 操作台、多模态路由、SQLite 记忆与历史存储、任务调度、外部消息和模块化更新已经形成完整闭环；当前版本将记忆、历史与运行状态统一迁移到每用户独立的 SQLite 数据库，并加固 Provider 工具调用完整性边界。项目仍处于早期阶段，兼容性与扩展体验会继续演进。

接下来可以阅读 [v0.10.0 更新说明](/releases/release-0.10.0)，或直接进入[快速开始](/guide/getting-started)。
