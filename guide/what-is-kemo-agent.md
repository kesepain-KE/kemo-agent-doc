...
项目当前正式版本为 `0.9.2`。核心运行、Web 操作台、多模态路由、分层记忆、任务调度、外部消息和模块化更新已经形成完整闭环；当前版本进一步修复后台记忆子代理的多轮 reasoning 标识冲突，并增强 Windows 长上下文压缩的原子提交可靠性。项目仍处于早期阶段，兼容性与扩展体验会继续演进。

接下来可以阅读 [v0.9.2 更新说明](/releases/release-0.9.2)，或直接进入[快速开始](/guide/getting-started)。
