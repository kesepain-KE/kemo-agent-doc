...
加权归属的日期不是「处理发生的那一天」，而是**原轮次成功提交的那一天**（按北京时间换算）。所以补提取、后台延迟处理或乱序到达都不会改变加权归属——证据算在哪一天，取决于对话本身发生在哪一天。

碎片首次建立时权重为零，只登记一条创建记录占据当天的日锁，不计入加分。当前档位的创建日是接纳历史证据的下界：比它更早的乱序证据会被保守跳过，既不重置生命期，也不倒推创建时间。普通晋升清空原档位事件并把权重归零；拆分晋升则沿用继承的进入时间作为下界。
