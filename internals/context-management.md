...
历史归档、Provider 临时工作区、上下文摘要缓存和会话索引都先写入同目录临时文件，刷新到磁盘后再原子替换目标文件。Windows 安全软件、索引器或备份程序短暂占用目标文件时，运行时会对明确的共享冲突做有限退避重试；磁盘已满、路径无效和持续权限错误仍会直接返回。

自动压缩只有在归档和压缩后的临时工作区都成功提交后才会在下一轮生效。若短暂占用在重试期限内解除，提交继续完成且 `data.json.complete` 保持为 `true`；若错误持续存在，系统不会把未完成窗口伪装成成功压缩。
